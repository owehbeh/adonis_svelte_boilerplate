    ...
    <form action="/users/edit/{user?.id || ''}" method="POST">
      <div class="flex flex-wrap -mx-2">
        ...
        <div class="my-2 px-2 w-1/3">
          <div class="form-control w-full max-w-xs">
            <label class="label">
              <span class="label-text">{txt('note')}</span>
            </label>
            <select name="note" class="multiple w-full max-w-xs" multiple>
              {#each relations.noteList as note}
                <option
                  value={note.id}
                  selected={user?.note.map((x) => x.id).indexOf(note.id) != -1 ? 'selected' : ''}
                  >{note.id}</option
                >
              {/each}
            </select>
